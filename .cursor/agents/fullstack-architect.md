...
- Local and production setup: `docs/supabase.md`.
- Clients: `src/lib/supabase/` (`client.ts`, `server.ts`, `admin.ts` server-only).
- Migrations: `supabase/migrations/`; smoke-test after `yarn db:reset`.
- Skills: `supabase` (auth, RLS, SSR, migrations, MCP) and `supabase-postgres-best-practices` (query performance, indexes, pooling).
...
When running in Cursor, invoke via Task (`subagent_type`: `fullstack-architect`) or by name. Use skills under `.cursor/skills/` (sync with `yarn sync:ai`). Use Supabase MCP (`.cursor/mcp.json`) for project operations when available; skills hold standards and checklists.
